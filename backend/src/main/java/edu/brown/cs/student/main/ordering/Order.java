...
public class Order {
  ...
  public ArrayList<HashMap<String, String>> order(
      HashMap<String, HashMap<String, String>> database, HashMap<String, String> target) {
  ...
  public Double compare(HashMap<String, String> comparison, HashMap<String, String> target) {
    ...
    if (Integer.parseInt(comparison.get("year")) == Integer.parseInt(target.get("year"))) {
      ...
      double difference =
          Integer.parseInt(target.get("year")) - Integer.parseInt(comparison.get("year"));
      if (difference > 0) {
        difference *= -1;
      }
    ...
    // score = cosine.similarity(comparison.get("overview").toString(),
    // target.get("overview").toString());
    return score;
  }
